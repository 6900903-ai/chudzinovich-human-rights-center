...
const fresh=news.filter(item=>{const t=new Date(item.published_at).getTime();return Number.isFinite(t)&&t<=now+5*60*1000&&now-t<=twoDays;}).slice(0,1000);
...
console.log(`LIVE_NEWS_SEO_FINALIZE=PASS rss_entries=${Math.min(news.length,100)} total_news=${news.length} fresh_news_sitemap=${fresh.length} locales=4`);
