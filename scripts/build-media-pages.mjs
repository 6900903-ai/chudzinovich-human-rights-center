...
const sources = [...registry.sources].sort((a,b) => {
  const active = Number(b.candidate_discovery_enabled === true) - Number(a.candidate_discovery_enabled === true);
  return active || String(a.name).localeCompare(String(b.name),'en');
});
const enabled = sources.filter(x => x.candidate_discovery_enabled === true);
const langs = ['ru','be','en','pl'];

const COPY = {
  ru:{title:'СМИ и медиапроекты',intro:'Публичный каталог зарегистрированных медиа-источников CHUDO. Наличие источника в каталоге означает, что система знает его происхождение; это не означает автоматического подтверждения каждого опубликованного им утверждения.',all:'Всего источников',active:'Активно для discovery',materials:'Материалы источника',empty:'Опубликованных материалов этого источника пока нет.',site:'Открыть сайт',back:'Все СМИ',status:'Статус мониторинга',class:'Класс источника',weight:'Роль в подтверждении',domains:'Домены',enabled:'Используется для поиска новых сообщений',disabled:'Сохранён в реестре, автоматический discovery выключен'},
  be:{title:'СМІ і медыяпраекты',intro:'Публічны каталог зарэгістраваных медыя-крыніц CHUDO. Наяўнасць крыніцы ў каталогу азначае, што сістэма ведае яе паходжанне; гэта не азначае аўтаматычнага пацверджання кожнага сцвярджэння.',all:'Усяго крыніц',active:'Актыўна для discovery',materials:'Матэрыялы крыніцы',empty:'Апублікаваных матэрыялаў гэтай крыніцы пакуль няма.',site:'Адкрыць сайт',back:'Усе СМІ',status:'Стан маніторынгу',class:'Клас крыніцы',weight:'Роля ў пацвярджэнні',domains:'Дамены',enabled:'Выкарыстоўваецца для пошуку новых паведамленняў',disabled:'Захавана ў рэестры, аўтаматычны discovery выключаны'},
  en:{title:'Media sources',intro:'Public directory of registered CHUDO media sources. Inclusion means the system knows the source and its provenance; it does not automatically verify every claim published by that source.',all:'Total sources',active:'Discovery enabled',materials:'Source materials',empty:'No published materials from this source yet.',site:'Open website',back:'All media',status:'Monitoring state',class:'Source class',weight:'Corroboration role',domains:'Domains',enabled:'Used for discovery of new reports',disabled:'Kept in the registry; automatic discovery is disabled'},
  pl:{title:'Źródła medialne',intro:'Publiczny katalog zarejestrowanych źródeł medialnych CHUDO. Obecność w katalogu oznacza znane pochodzenie źródła; nie oznacza automatycznego potwierdzenia każdego jego twierdzenia.',all:'Wszystkie źródła',active:'Aktywne w discovery',materials:'Materiały źródła',empty:'Brak opublikowanych materiałów z tego źródła.',site:'Otwórz stronę',back:'Wszystkie media',status:'Stan monitoringu',class:'Klasa źródła',weight:'Rola w potwierdzaniu',domains:'Domeny',enabled:'Używane do wyszukiwania nowych doniesień',disabled:'Zachowane w rejestrze; automatyczne discovery wyłączone'}
};
...
console.log(`MEDIA_DIRECTORY_BUILD=PASS sources=${sources.length} enabled=${enabled.length}`);
