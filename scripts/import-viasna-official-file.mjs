...
await Promise.all([
  ...
  mkdir(preparedRoot,{recursive:true,mode:0o700}),
  mkdir(receiptsRoot,{recursive:true,mode:0o700})
]);
...
const prepared=await prepareViasnaSnapshot({
  ...
  currentPublicDir:join(repoRoot,'data','public','current'),
  sourcePageUrl,
  locale,
  asOf
});
...
const receipt={
  state:'PREPARED_FOR_PRIVATE_REVIEW_NOT_PUBLISHED',
  ...
  prepared_run_id:prepared.runId,
  candidate_snapshot_id:prepared.snapshotId,
  candidate_snapshot_manifest_sha256:candidateManifestSha256,
  people:prepared.people,
  prisons:prepared.prisons,
  quarantined_rows:prepared.quarantined,
  quarantine_ratio:quarantineRatio,
  public_repo_mutated:false,
  production_published:false,
  next_gate:'PRIVATE_EDITORIAL_REVIEW_AND_EXPLICIT_SNAPSHOT_PROMOTION'
};
await writeFile(join(receiptsRoot,`${receiptId}.json`),JSON.stringify(receipt,null,2)+'\n',{encoding:'utf8',mode:0o600,flag:'wx'});

console.log(`VIASNA_OFFICIAL_IMPORT=PASS state=${receipt.state} rows=${receipt.parsed_rows} people=${receipt.people} prisons=${receipt.prisons} quarantined=${receipt.quarantined_rows} source_sha256=${sourceSha256}`);
