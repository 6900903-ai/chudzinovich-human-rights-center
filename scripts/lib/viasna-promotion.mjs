...
export function promoteViasnaObservations(observations,{existingPeople=[],asOf=new Date().toISOString()}={}){
  ...
  for(const observation of partition.clean){
    ...
    if(detDate)person.detentions.push({event_id:eventId('det',personId,observation.row_number),date:detDate,summary:{ru:'Дата задержания по данным «Вясны».',be:'Дата затрымання паводле «Вясны».',en:'Detention date according to Viasna.',pl:'Data zatrzymania według Viasny.'},location:null,source_id:'src-viasna',source_url:sourceUrl,source_observed_at:observation.source_observed_at,state:'ACTIVE'});
    if((observation.charge_articles||[]).length||observation.charges_raw)person.charges.push({event_id:eventId('charge',personId,observation.row_number),articles:observation.charge_articles||[],summary:localized(observation.charges_raw||''),source_id:'src-viasna',source_url:sourceUrl,source_observed_at:observation.source_observed_at,state:'ACTIVE'});
    ...
    if(observation.sentence_raw||observation.penalty_raw)person.sentences.push({event_id:eventId('sen',personId,observation.row_number),date:verdictDate,value:localized([observation.sentence_raw,observation.penalty_raw].filter(Boolean).join(' · ')),source_id:'src-viasna',source_url:sourceUrl,source_observed_at:observation.source_observed_at,state:'ACTIVE'});
    if(observation.prison?.facility){const pid=prisonId(observation.prison.facility,observation.prison.address);const prison={prison_id:pid,name:localized(observation.prison.facility),address:localized(observation.prison.address||''),source_id:'src-viasna',source_url:sourceUrl,publication_state:'PUBLIC_SOURCE_ATTRIBUTED'};prisonsById.set(pid,prison);person.prison_placements.push({event_id:eventId('place',personId,observation.row_number),started_at:verdictDate||detDate,current:status==='POLITICAL_PRISONER'&&!releaseDate&&!observation.release_claim,prison_id:pid,prison_name:localized(observation.prison.facility),source_id:'src-viasna',source_url:sourceUrl,source_observed_at:observation.source_observed_at,state:'ACTIVE'});}
    if(releaseDate||observation.release_claim)person.release_events.push({event_id:eventId('rel',personId,observation.row_number),date:releaseDate,summary:{ru:'Источник указывает, что человек освобождён.',be:'Крыніца пазначае, што чалавек вызвалены.',en:'The source lists the person as released.',pl:'Źródło wskazuje, że osoba została zwolniona.'},source_id:'src-viasna',source_url:sourceUrl,source_observed_at:observation.source_observed_at,state:'ACTIVE'});
    person.status_events.push({event_id:eventId('status',personId,observation.row_number),status,state:'ACTIVE',designation:'SOURCE_ATTRIBUTED',effective_date:null,source_id:'src-viasna',source_url:sourceUrl,source_published_at:null,source_observed_at:observation.source_observed_at,human_verified_at:null,recognized_ex_post_facto:Boolean(observation.source_status_claim?.recognized_ex_post_facto),summary:statusSummary(type)});
    people.push(person);
  }
