...
export function detectObservationAnomalies(observations, { asOf = new Date().toISOString(), expectedCount = null } = {}) {
  ...
  const seenSourceRecord = new Map();

  for (const observation of observations) {
    if (!observation.reported_name) anomalies.push({ severity:'HIGH', code:'MISSING_REPORTED_NAME', row_number:observation.row_number });
    ...
    if (recordId) {
      if (seenSourceRecord.has(recordId)) anomalies.push({ severity:'HIGH', code:'DUPLICATE_SOURCE_RECORD_ID', row_number:observation.row_number, first_row_number:seenSourceRecord.get(recordId), source_record_id:recordId });
      else seenSourceRecord.set(recordId, observation.row_number);
    }

    const key = observation.source_identity_key;
    if (key) {
      if (seenIdentity.has(key)) anomalies.push({ severity:'HIGH', code:'DUPLICATE_SOURCE_IDENTITY', row_number:observation.row_number, first_row_number:seenIdentity.get(key) });
...
export function compareSnapshotMetrics(previous, current, { maxRelativeDrop=0.10, minAbsoluteDrop=50, maxMissingPrisonRateIncrease=0.25 } = {}) {
  const anomalies = [];
  if (previous?.total > 0 && current?.total >= 0) {
    const drop = previous.total - current.total;
    const ratio = drop / previous.total;
    if (drop >= minAbsoluteDrop && ratio > maxRelativeDrop) anomalies.push({ severity:'BLOCK', code:'ANOMALOUS_TOTAL_DROP', previous:previous.total, current:current.total, drop, ratio });
  }
  if (previous && current) {
    const increase = (current.current_missing_prison_rate ?? 0) - (previous.current_missing_prison_rate ?? 0);
    if (increase > maxMissingPrisonRateIncrease) anomalies.push({ severity:'BLOCK', code:'MISSING_PRISON_RATE_SPIKE', previous_rate:previous.current_missing_prison_rate ?? 0, current_rate:current.current_missing_prison_rate ?? 0, increase });
  }
  return anomalies;
}
