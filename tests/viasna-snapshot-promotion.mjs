...
try{
  ...
  const noAuth=spawnSync(process.execPath,[join(root,'scripts/promote-viasna-snapshot.mjs')],{
    encoding:'utf8',
    env:{...process.env,CHRC_TEST_MODE:'1',CHRC_VIASNA_CANDIDATE_DIR:prepared.snapshotDir,CHRC_VIASNA_AUDIT_RECEIPT_FILE:auditReceipt,CHRC_VIASNA_PROMOTION_TARGET_DIR:noAuthTarget}
  });
  ...
  console.log('VIASNA_SNAPSHOT_PROMOTION_TEST=PASS authorization=PASS audit_binding=PASS candidate_binding=PASS support_data_drift=PASS atomic_target=PASS repository_mutation=ZERO');
}finally{
