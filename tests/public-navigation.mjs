...
for(const href of ['/search/','/sources/','/media/','/channels/','/press/','/methodology/','/corrections/','/contacts/','/privacy/','/security/','/terms/'])assert.ok(ru.includes(`href="${href}"`),`RU shell missing ${href}`);
for(const href of ['/be/search/','/be/sources/','/be/media/','/be/channels/','/be/press/','/be/methodology/','/be/corrections/','/be/contacts/','/be/privacy/','/be/security/','/be/terms/'])assert.ok(be.includes(`href="${href}"`),`BE shell missing ${href}`);
...
assert.equal((ru.match(/CHUDO_PUBLIC_NAV_V5/g)||[]).length,1);
assert.ok(ru.includes('Публичный правозащитный архив'));
assert.ok(be.includes('Публічны праваабарончы архіў'));
console.log('PUBLIC_NAVIGATION_TEST=PASS utility_links=11 locales=4 footer=PASS');
