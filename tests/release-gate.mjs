...
assert.ok(report.blocking.some(item => item.name === 'VIASNA_SOURCE_ACQUISITION_GATE'));
assert.match(report.configured_gates.VIASNA_SOURCE_ACQUISITION_GATE,/PENDING_REAL_EXPORT_OR_CLEARED_ENDPOINT/);
const enforce = spawnSync(process.execPath,[script,'--enforce'],{encoding:'utf8'});
assert.equal(enforce.status,2);
console.log('RELEASE_GATE_TEST=PASS fail_closed=true viasna_acquisition_blocked=true');
